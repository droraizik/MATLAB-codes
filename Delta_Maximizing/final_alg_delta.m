...
num_elems=1;
element_location = create_element_matrix(N2,num_elems,[20 :9:55]);
...
camera_int = activate_vector_only_FFT(forward_TM,backward_TM,SLM_patt,N1,1,forward_TM_show,N2,elems_c,elems_r);
...
%         delta_ints(2*i-1)=sum(sum(abs(camera_int(99:101,99:101)).^2));
        ...
        SLM_patt_new=sign(SLM_patt_new).*SLM_patt;
        camera_int = activate_vector_only_FFT(forward_TM,backward_TM,SLM_patt_new,N1,0,forward_TM_show,N2,elems_c,elems_r);
%         delta_ints(2*i)=sum(sum(abs(camera_int(99:101,99:101)).^2));
